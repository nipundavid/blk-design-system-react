export const myProjects = [
  {
    id: 1,
    projectName: "Color Jump 3D",
    thumbImage: require("assets/img/thumb_ColorJump3D.jpg"),
    techStack: ["Unity3D, C#, PhotoShop, Blender3d"],
    tags: ["#game #unity3D #3d"],
    shortDescription:
      "Tap the ball through each obstacle so that the color of ball and obstacle should be same",
    longDescription: "",
  },
  {
    id: 2,
    projectName: "Shadow Run",
    thumbImage: require("assets/img/thumb_ShadowRun.png"),
    techStack: ["Unity3D, C#, PhotoShop, Blender3d"],
    tags: ["#game #unity3D #3d"],
    shortDescription:
      "Save your balls from spikes and enjoy party music. Game-play is mind boggling with single tap on left and right of the screen",
    longDescription: "",
  },
  {
    id: 3,
    projectName: "Re-Usable UI For Unity3D",
    thumbImage: require("assets/img/thumb_ReUsableUIForUnity3D.png"),
    techStack: ["Unity3D, C#"],
    tags: ["#app #unity3D #3d #opensource"],
    shortDescription:
      "A framework that can enable quick prototyping of UI screens alongwith smooth transition",
    longDescription: "",
  },
  {
    id: 4,
    projectName: "360 Viewer",
    thumbImage: require("assets/img/thumb_360xp.png"),
    techStack: ["ReactJS, BabylonJS, HTML"],
    tags: ["#360 #VR #3d"],
    shortDescription:
      "The project aims to create immersive experience with the help of 360-degree photographs",
    longDescription: "",
  },
];
